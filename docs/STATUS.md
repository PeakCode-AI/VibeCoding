...
共享：PostgreSQL `vibe` 库 + Redis；`vibe_common` 双份 vendored（Base ↔ Admin，由 `scripts/check_vibe_common_sync.sh` 校验）。
...
| 官方支付网关 RSA/证书 | 现有 HMAC notify + 开发 callback 即可联调 |
