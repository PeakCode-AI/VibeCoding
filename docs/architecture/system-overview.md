...
| 支付中台 | **VibePay** | 运营 / 商户 | 免签约微信/支付宝收款、订单匹配、异步回调入账，商业闭环的「收钱」环节 |
...
### VibePay（支付中台）
- **补齐商业闭环的「收钱」环节**：VibeBase / VibeApp / Vibe-Mp-H5 的充值下单后，由 VibePay 生成收款二维码并承接支付，再通过安卓监控端监听收款通知、回调 VibeBase 完成积分入账。
- 基于 Spring Boot 2.1.1 + JPA + PostgreSQL，**多租户 `tenant_id` 行级隔离**，并进一步支持「公司级（Corporate）+ 应用级（App）」两层接入模型，适合个人收款与多商户 SaaS 部署。
- 提供完整的开放 API（创建订单 / 查询 / 状态 / 关闭 / 异步回调），通讯密钥签名验证、订单超时、金额去重、幂等回调等多重资金安全保障。
- 已部署线上站点 [https://pay.vibeadmin.cn/](https://pay.vibeadmin.cn/)，配套安卓监控端 `vibePay-App/` 负责监听微信/支付宝收款通知栏并上报。

## 3. 技术栈对照

| 维度 | VibeAdmin | VibeBase | VibeApp | Vibe-Mp-H5 | VibePay |
| --- | --- | --- | --- | --- | --- |
| 前端框架 | Vue 3.5 + Vite 7 + TS 5.8 | Vue 3 + TS | Flutter / Dart | uni-app + Vue 3 + Vite + TS | 原生 HTML + 安卓监控端 |
| UI | Tailwind 4 + shadcn-vue | Vue 3 + TS | 40+ 内置组件 | Wot UI + UnoCss | 管理后台 + 收银台页面 |
| 后端 | FastAPI + Uvicorn | FastAPI | — | — | Spring Boot 2.1.1 (JPA) |
| 状态/数据 | Pinia 3 + TanStack Query 5 | — | Riverpod + GoRouter | Pinia | JPA / Hibernate Filter |
| HTTP 客户端 | axios | Dio | Dio | alova | RestTemplate / OkHttp |
| 数据库 | PostgreSQL（统一主库） | PostgreSQL（统一主库） | SharedPreferences | — | PostgreSQL（独立库，多租户） |
| 缓存 | Redis（令牌黑名单 / 限流） | Redis（限流） | — | — | — |
| 认证 | JWT (python-jose + bcrypt) | JWT (python-jose + bcrypt) | — | — | 通讯密钥签名 / JWT (租户) |
...
- **支付闭环**：VibeBase 充值下单 → 调用 VibePay 创建订单并生成收款二维码 → 用户微信/支付宝付款 → 安卓监控端（`vibePay-App`）监听通知栏并上报 VibePay → VibePay 匹配订单并异步回调 VibeBase `/api/v1/recharge/notify` → 积分自动入账。VibePay 使用独立 PostgreSQL 实例，与业务库解耦，仅通过回调接口对接。
- 各后端独立部署，可通过 Nginx 反向代理（如 VibeAdmin 的 `/api/`、VibePay 的 `pay.vibeadmin.cn`）聚合对外。
...
| VibePay 后端 | 8080（线上 `pay.vibeadmin.cn`） | Spring Boot 支付中台 |
| VibePay PostgreSQL | 5432（独立实例 `vibepay`） | 支付订单/租户/二维码库 |
